...
import { Calendar, ClipboardList, Crown, FileCheck2, Gift, HelpCircle, Home, Loader2, User as UserIcon } from 'lucide-react';
    ...
    <div className="size-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-auto px-4 pt-24 pb-8">
        {dashboard.dataLoading ? (
          <div className="mx-auto flex max-w-md flex-col items-center justify-center rounded-2xl border border-green-100 bg-green-50 px-6 py-12 text-center">
            <Loader2 className="h-8 w-8 animate-spin text-green-700" />
            <span className="mt-3 font-semibold text-green-900">Memuat dashboard relawan</span>
            <span className="mt-1 text-sm text-green-700">Mengambil kegiatan, laporan, sertifikat, reward, dan leaderboard kamu.</span>
          </div>
        ) : (
          <>
            {activePage === 'home' && (
            ...
            {activePage === 'help' && (
              <UserHelp user={user} onNavigate={setActivePage} />
            )}
